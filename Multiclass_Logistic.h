...
namespace sklearn_cpp {
    namespace linear_model {
        ...
        class SoftmaxRegression : public BaseModel {
            ...
            protected:
                ...
                void compute_softmax_gradients(const std::vector<std::vector<double>>& X, const std::vector<double>& Y, std::vector<std::vector<double>>& grad_w, std::vector<double>& grad_b) {
                    for (size_t i = 0; i < X.size(); i++) {
                        ...
                        for (int k = 0; k < K; k++) {
                            double error = prob[k] - (Y[i] == k ? 1.0 :0.0);
                            for (size_t j = 0; j < X[i].size(); j++) {
            ...
            public: 
                SoftmaxRegression(int n_features,int K, LossModel& loss, double lr = 0.01, int iter = 1000): BaseModel{n_features, lr, iter}, loss_model{loss}, K{K} {
                    w.resize(K, std::vector<double>(n_features, 0.0));
                    b.resize(K,0.0);
                }
                
                void fit(const std::vector<std::vector<double>>& X, const std::vector<double>& Y) {
                    for (int i = 0; i < iterations; i++) {
                        std::vector<std::vector<double>> grad_w(K, std::vector<double>(w[0].size(), 0.0));
                        ...
                        for (int k = 0; k < K; k++) {
                            for (size_t j = 0; j < w[k].size(); j++) {
                                w[k][j] -= learning_rate * grad_w[k][j];
                            };
